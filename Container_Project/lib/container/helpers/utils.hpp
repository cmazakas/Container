...
template <typename F> auto spinLockExecutorHelper( const F& f, std::atomic_flag& lock, Tag<void> ) -> void
{
  spinLock( lock );
  f();
}

template <typename F, typename R> auto spinLockExecutorHelper( const F& f, std::atomic_flag& lock, Tag<R> ) -> R
{
  spinLock( lock );
  return f();
}

template <class F> auto spinLockExecutor( const F& f, std::atomic_flag& l ) -> decltype( f() )
{
 ...
 * */
template <class T> void linkElements( Element<T>* a, Element<T>* b )
{
  a->setNext( b );
  b->setNext( a );
}
